...
public class RecipeController : BaseNetLogic
{
    ...
    const string italianLocale = "it-IT";
    const string germanLocale  = "de-DE";
    const string spanishLocale = "es-ES";
    ...
    private string GetLocalizedTextString(string textId)
	{
		...
		return InformationModel.LookupTranslation(localizedText, new List<string>() { Session.ActualLocaleId, englishLocale }).Text;
	}

    private void AddTranslationIfNeeded(string key, string italianTranslation, string englishTranslation, string germanTranslation, string spanishTranslation, string frenchTranslation)
	{
        ...
        InformationModel.AddTranslation(new LocalizedText(Project.Current.NodeId.NamespaceIndex, key, "", Session.ActualLocaleId));

		if (Project.Current.Locales.Contains(englishLocale))
		{
			var localizedTextEnglish = new LocalizedText(Project.Current.NodeId.NamespaceIndex, key, englishTranslation, englishLocale);
			InformationModel.SetTranslation(localizedTextEnglish);
		}

		if (Project.Current.Locales.Contains(italianLocale))
		{
			var localizedTextItalian = new LocalizedText(Project.Current.NodeId.NamespaceIndex, key, italianTranslation, italianLocale);
			InformationModel.SetTranslation(localizedTextItalian);
		}

		if (Project.Current.Locales.Contains(germanLocale) && germanTranslation != "")
		{
			var localizedTextGerman = new LocalizedText(Project.Current.NodeId.NamespaceIndex, key, germanTranslation, germanLocale);
			InformationModel.SetTranslation(localizedTextGerman);
		}

		if (Project.Current.Locales.Contains(spanishLocale) && spanishTranslation != "")
		{
			var localizedTextSpanish = new LocalizedText(Project.Current.NodeId.NamespaceIndex, key, spanishTranslation, spanishLocale);
			InformationModel.SetTranslation(localizedTextSpanish);
		}

		if (Project.Current.Locales.Contains(frenchLocale) && frenchTranslation != "")
		{
			var localizedTextFrench = new LocalizedText(Project.Current.NodeId.NamespaceIndex, key, frenchTranslation, frenchLocale);
			InformationModel.SetTranslation(localizedTextFrench);
		}
	}

    private void AddTranslations()
	{
                              //         key           ,   it-IT  ,  en-US  , de-DE, es-ES, fr-FR 
		AddTranslationIfNeeded("RecipeControllerRecipe", "Ricetta", "Recipe", "", "", "");
		AddTranslationIfNeeded("RecipeControllerSaved", "salvata", "saved", "", "", "");
		AddTranslationIfNeeded("RecipeControllerCreatedAndSaved", "creata e salvata", "created and saved", "", "", "");
		AddTranslationIfNeeded("RecipeControllerRecipeLoaded", "Ricetta caricata", "Recipe loaded", "", "", "");
        AddTranslationIfNeeded("RecipeControllerRecipeApplied", "Ricetta applicata", "Recipe applied", "", "", "");
        AddTranslationIfNeeded("RecipeControllerEmptyRecipeName", "Il nome della ricetta è vuoto", "Recipe name is empty", "", "", "");
        AddTranslationIfNeeded("RecipeControllerDeleted", "eliminata", "deleted", "", "", "");
	}
    #endregion
